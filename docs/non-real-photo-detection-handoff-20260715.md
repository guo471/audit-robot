...
必须注意：历史单图V4/FFT拦截率和实拍干预率只是专项方案的参考数据，不能证明合并后的第二次模型调用会获得相同准确率。合并提示词的上下文更长、任务更多，模型输出可能变化。当前安全措施是随时切回 `off`，并让异常或命中的订单进入人工；这些措施降低业务风险，但不能替代真实运行观察。后续必须按实际订单持续统计漏放、新增转人工、字段异常、耗时和token，再判断合并方案是否稳定。
