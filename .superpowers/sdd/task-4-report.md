...
### Elapsed-value fail-closed validation

A follow-up review found that invalid elapsed values could enter accounting: `_number` converted booleans to zero, normal attempts retained negative/non-finite values, and network failures floored negative values instead of rejecting the source data.

TDD evidence:

- RED command: `python -m pytest tests\test_guobu_audit_report.py -k 'attempt_elapsed_must or invalid_elapsed_accounting or invalid_attempt_elapsed' -q`
- RED result: `24 failed, 131 deselected`; normal and network attempts plus accounting totals and trace fields all accepted negative, NaN, Infinity, or boolean elapsed values.
- Focused GREEN result: `24 passed, 131 deselected in 0.41s`.
